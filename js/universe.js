/* ==========================================================
   ...
   · Each galaxy is a photographic picture computed by cosmos.js (bright
     core, clumpy arms, dust lanes, pink star-forming knots), with a few
     thousand real stars on top that you fly between when you get close.
     Until its picture is ready (a second or two, in the background) a
     galaxy is drawn from its stars alone, then the picture fades in.
...
const Universe=(function(){
  ...
  /* each has its own colours: core (the bulge), stars (the disk, [colour, weight]) and glow (inner, outer);
     tex: how cosmos.js paints its picture (see cosmos.js) */
  const GALAXIES={
    ...
    nolan:    {kind:"oval",      r:10,  n:6500, tilt:1.10, roll:-.50, at:{d:[-.8,.78],m:[-.95,.82]},  z:30,
               ...
               texA:.62, tex:{seed:11,speck:.12,grain:.004,core:"255,208,150",disk:"255,228,200",arm:"205,215,255",knot:"255,150,190",bulge:1.6,bulgeR:.13,bulgeQ:.8,sersic:2,disk0:.8,h:.26,armAmp:.4,pitch:.34,knots:.3,dust:1.3}},
    ...
    forge:    {kind:"elliptical",r:4.2, n:3200, tilt:.6,   roll:-.2,  at:{d:[-.66,-.5],m:[-.62,-.74]}, z:75,
               ...
               texA:.8, tex:{seed:37,core:"255,214,170",disk:"255,190,130",arm:"255,190,130",knot:"0,0,0",Ie:.11,bulgeR:.26,bulgeQ:.8,sersic:4,disk0:0,h:.3,armAmp:0,knots:0,dust:0,exposure:1.5}},
  ...
  let world={};             /* per galaxy: centre and stars in world coordinates, and a picture of its stars */
  function place(){
    ...
    IDS.forEach(id=>{
      ...
      world[id]={id,g,cx,cy,cz:g.z,r:g.r,pts,sprite:sprite(g,pts,cx,cy)};
    });
  ...
  const tex={};             /* id → {img, at (when it arrived)} */
  let fadeRaf=0;
  const FADE=1400;
  const texAlpha=t=>t?Math.min(1,(performance.now()-t.at)/FADE):0;
  function fadeLoop(){
    fadeRaf=0;
    if(!anim) draw();
    const busy=Object.values(tex).some(t=>texAlpha(t)<1)||deep.some(d=>d.img&&texAlpha(d)<1);
    if(busy) fadeRaf=requestAnimationFrame(fadeLoop);
  }
  function arrived(obj,img){
    obj.img=img; obj.at=fancy()?performance.now():-1e9;     /* without animations it simply appears */
    if(!fadeRaf) fadeRaf=requestAnimationFrame(fadeLoop);
  }
  function paint(){
    if(typeof Cosmos==="undefined") return;
    const big=Math.min(W,H)>=600;
    /* home's galaxy first: it is the one you see right after the PIN */
    IDS.forEach(id=>{
      const g=GALAXIES[id];
      const size=id==="nolan"?(big?1024:768):(big?768:512);
      Cosmos.galaxy({speck:size>600?.2:.44,...g.tex,size,tilt:g.tilt,roll:g.roll}).then(img=>{ tex[id]={}; arrived(tex[id],img); });
    });
    /* then a handful of tiny galaxies for the deep field */
    DEEP_KINDS.forEach((p,i)=>Cosmos.galaxy(p).then(img=>{ deepImg[i]=img; deep.filter(d=>d.kind===i).forEach(d=>arrived(d,img)); }));
  }
  ...
  function draw(fieldBoost=0){
    ...
    for(const w of order){
      ...
      const t=tex[w.id], ta=texAlpha(t);
      ...
      if(starA>0){
        ...
        for(const p of w.pts){
          ...
          x.globalAlpha=starA*p.b*Math.min(1,.3+kk*.004)*Math.min(1,pz/1.2);
  ...
  return {go, scene:()=>scene, busy:()=>!!anim, painted:()=>Object.keys(tex).length, GALAXIES};
})();
