...
- **Further down**, where to go:
  ...
  - **"Explore the universe"**: one card; it opens a drop-down list of the places (`#placeMenu`, written by `home.js` from `SIGHTS`): each with its icon and colour, its name and a few words (the Moon's follow its phase tonight). Choosing one flies there. Hidden without the 3D universe.
- **A sight's page** (`#sightView`, route `#<id>`): the camera flies there, the hero steps aside and the words sit low over a soft shade, so the sight fills the sky above them: a short line on what it is, its name (a button: it opens the same list of places, the one you are at marked; Escape closes it), a few words, a dot for each sight (this one lit), and Nolan's logo back to home. The arrows to the previous and the next sight (the list goes round) are fixed at the two edges of the screen, level with the sight (`#sightNav`), so they are in the same place for every sight; the arrow keys ← → do the same. On a short screen (a phone on its side) the few words are left out, so they do not cover the sight. Back on home, the list is where you left it.
