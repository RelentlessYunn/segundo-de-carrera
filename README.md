...
- **A black hole** (`SIGHTS` in `universe.js`), seen from home and with a window of its own: its card on home (route `#blackhole`) flies the camera straight up to it, the text sits low and the hole fills the sky. It is drawn in the last step of each frame, where it bends the light of everything behind it (a point lens): a dark shadow hugged by two thin rings of light that went round it, and a swirling hot disk seen a little from above — its near half crosses in front of the shadow, its far half is bent up over the top (and a thin image under it). The side of the disk turning towards us is brighter and bluer, the other side dimmer and redder, and the gas at the inner edge is dimmed and reddened by the hole's pull. Its disk leans against a fixed axis, so it is seen from above both from home and up close.
- **Shooting stars and comets**: a shooting star every few seconds (now and then two together), starting anywhere and crossing in any direction: a hot blue-green head with a soft halo and a tapering tail that cools to orange, glows and flickers, and flares a little before it dies. Comets cross the screen in any direction, from beyond one edge to beyond the other, often two or three at once: most drift across for about a minute, some shoot across in ten or fifteen seconds. Each has a bright nucleus in a green coma, a broad curved dust tail with faint rays and a straight blue ion tail streaming away. Shooting stars and comets glow up and fade away slowly, never popping in or out. Only with Animations = All.
...
| **Settings** (`#settings`, inside home) | Language (Spanish / English), animations (all / basic / none) and quality (high / medium / low). Saved on each device. Animations = None plus Quality = Low turns every effect off. There is only the dark look (the light theme was removed in v0.52). Changing any of them reloads the page through a passage: the screen fades softly (≈0.4 s) to the colour of the empty night sky, not a flat black, the page reloads behind it and the new look fades in (`js/shift.js`). The fades run on the compositor, so the busy start of the page behind cannot make them stutter; while the passage covers the screen completely (`html.shift-dark`) the universe draws nothing but the one frame the passage waits for. The passage opens when the page is ready — the universe built and drawn, the saved data read, the font in — but never waits more than 2.5 s: a galaxy still being built then fades in by itself. |
