...
- **Other wonders** (`js/wonders.js`), each drawn from a formula by the graphics card, in its place in space (so flights and the slow turn move them like everything else), all visible from home: the **Orion Nebula** (pink hydrogen lit by the four Trapezium stars, a veil of dust filaments); the **Pillars of Creation** (three dark columns with gold-lit edges in teal gas); the **Pleiades** (nine blue-white stars at their real places, with spikes, in fine streaks of blue reflection nebula); the **Ring Nebula** (teal inside, red rim, the white dwarf in the middle); a **red giant feeding a white dwarf** (the giant pulled into a drop, a stream of gas curving onto a disk, the two orbiting in 70 s and hiding each other); the **Antennae** (two galaxies colliding: merging cores, dust lanes, pink knots of newborn stars, and two long curved tidal tails).
...
- Every galaxy has its own shape in `GALAXIES`, after beautiful real ones: Nolan (home) a golden **barred spiral** (like NGC 1300: a straight bar of old stars, made by long aligned inner orbits and a cigar-shaped bulge, with two open arms); UC3M a blue **grand-design spiral** seen face-on (like the Whirlpool, M51: tightly wound arms full of pink knots, and a small yellow companion, NGC 5195), and the liveliest: it turns visibly while you look; Nolan-in-construction an amber **elliptical**; Andrómeda a **ring galaxy** (like Hoag's Object: a round yellow core, a dark gap and a nearly perfect ring of young blue stars); Sombrero **edge-on**, a big bright bulge cut by its dark ring of dust. On a phone its own scene is framed differently from the others (`whole` in its definition, `wholeView()` in `universe.js`): close up and from below, as the others are framed, the edge-on disk was wider than a phone and its dust ring became a black bar cut off by both edges, right across the header. There the camera stands nearly in its disk plane, a few degrees above it (the near side of the dust ring crosses the lower half of the bulge, as in the real one), far enough back for the whole disk to fit the width on any phone, and turns (a little yaw and pitch, never roll) so the galaxy sits high on the screen like the others; its slow turn circles it about its own axis, so it keeps that shape while it lives. `Universe.view(id)` says where a galaxy sits on the screen (tests).
...
- **To add a section** (a checklist, so nothing is forgotten):
  1. its galaxy in `GALAXIES` (`universe.js`): kind and shape, size, angle, colours, where it sits seen from home (`at.d` computer, `at.m` phone), and `star`, the colour of its bright star in the sky of stars (Effects = Medium) — `sky.js` reads it from there; if it is seen edge-on (or is otherwise wider than a phone close up), give it `whole` like the Sombrero;
...
| `wonders.js` | The other wonders drawn by the 3D universe: nebulae, the Pleiades, a feeding binary, the Antennae, supernovae, the Earth and the Moon. They register themselves in `window.UNIVERSE_EXTRAS`, so it loads before `universe.js`. |
...
| `astro.js` | The real sky, offline: sunrise and sunset, the Moon's phase (for the opening's Moon), the meteor shower going on (more shooting stars). |
...
Needs Node and Playwright. 93 checks in a real browser:

- the page loads without errors;
- the PIN screen (wrong PIN, the flight into the galaxy, remembered device, PIN not in the page, Log out), starting at home, the camera flights between galaxies (UC3M, back home, a galaxy to explore), the Sombrero seen whole and edge-on on three phone sizes, and Notes and Settings inside home;
...
- the astral layer: weather and sun on home, the Moon's phase and the meteor showers (checked against known dates), the band of our galaxy, the eight wonders compiled and drawn, a supernova, the Earth at the opening, the sea of stars, Quality = Low and the tasks constellation;
- the swipe gesture;
- idle;
