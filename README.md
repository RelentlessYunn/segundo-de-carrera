...
- **Other wonders** (`js/wonders.js`), each drawn from a formula by the graphics card, in its place in space (so flights and the slow turn move them like everything else), all visible from home: the **Orion Nebula** (pink hydrogen lit by the four Trapezium stars, a veil of dust filaments); the **Pleiades** (nine blue-white stars at their real places, with spikes, in fine streaks of blue reflection nebula); the **Ring Nebula** (teal inside, red rim, the white dwarf in the middle); a **red giant feeding a white dwarf** (the giant pulled into a drop, a stream of gas curving onto a disk, the two orbiting in 70 s and hiding each other); the **Antennae** (two galaxies colliding: merging cores, dust lanes, pink knots of newborn stars, and two long curved tidal tails).
...
- the astral layer: weather and sun on home, the Moon's phase and the meteor showers (checked against known dates), the band of our galaxy, the seven wonders compiled and drawn, a supernova, the Earth at the opening, the sea of stars, Quality = Low and the tasks constellation;
- the swipe gesture;
- idle;
